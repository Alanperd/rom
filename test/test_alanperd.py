...
if __name__ == '__main__':
    ...
    """
    before = datetime.now()
    objPerson= Person.query.filter(edad=21).all()
    num=objPerson= Person.query.filter(edad=21).count()
    after  = datetime.now()
    print after - before
    """
    """
    CountPersons=[]
    before = datetime.now()
    objPerson= Person.query.filter(edad=21).all(rowcount=CountPersons) 
    after  = datetime.now()
    print after - before
    """
